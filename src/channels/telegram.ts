...
import { maybeTranscribeVoice } from './telegram-voice.js';
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      // Transcribe voice notes first, so downstream code (pairing match, the
      // router, the agent) sees them as text. Errors are logged inside;
      // we always continue.
      if (message.kind === 'chat-sdk' && message.content && typeof message.content === 'object') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await maybeTranscribeVoice(message.content as any);
      }
